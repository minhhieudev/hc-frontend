...
const CustomTableHistoryPayment = <T extends { _id?: string }>(
  ...
  const getStyleColumnBase = (key: string): React.CSSProperties => {
    ...
    if (key === "total") return { textAlign: "right" };
    if (key === "increase") return { textAlign: "right" };
    if (key === "status") return { textAlign: "center" };
    if (key === "balance") return { textAlign: "right" };
    return {};
  };
    ...
    (row: any, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        case HeadCellBase.actions.key:
          return (
            <div className="flex flex-row justify-center items-center">v</div>
          );
...
export default CustomTableHistoryPayment;
